<!-- Intro -->
<article id="howto">
	<h2 class="major">@lang('landingpage.nav_how_to')</h2>
		<button class="accordion">@lang('landingpage.t_order')</button>
		<div class="panel">
		  	<div class="row">
				<div class="col-sm-3">
					<div class="service_block">
						<div class="service_icon">
							<span>
								<i class="fas fa-search">
								</i>
							</span>
						</div>
						<p class="text-center">@lang('landingpage.t_o1')</p>
					</div>
				</div>
				<div class="col-sm-3">
					<div class="service_block">
						<div class="service_icon">
							<span>
								<i class="fas fa-check">
								</i>
							</span>
						</div>
						<p>@lang('landingpage.t_o2')</p>
					</div>
				</div>
				<div class="col-sm-3">
					<div class="service_block">
						<div class="service_icon">
							<span>
								<i class="fas fa-money-bill">
								</i>
							</span>
						</div>
						<p>@lang('landingpage.t_o3')<br> + <br>@lang('landingpage.t_o4')</p>
					</div>
				</div>
				<div class="col-sm-3">
					<div class="service_block">
						<div class="service_icon">
							<span>
								<i class="fas fa-gift">
								</i>
							</span>
						</div>
						<p>@lang('landingpage.t_o5')</p>
					</div>
				</div>
			</div>
		</div>
		
		<button class="accordion">@lang('landingpage.t_ht_contact')</button>
		<div class="panel">
			<p>@lang('landingpage.t_ht_contact_1') <span><a href="#contact">@lang('landingpage.nav_contact')</a></span></p>
		</div>

		<button class="accordion">Women Shoes Size Chart</button>
		<div class="panel">
			<div class="table-wrapper">
				<h3>Women Shoes</h3>
				<table class="alt">
					<tr>
						<th>Korea (mm)</th>
						<td>220</td>
						<td>225</td>
						<td>230</td>
						<td>235</td>
						<td>240</td>
						<td>245</td>
						<td>250</td>
						<td>255</td>
						<td>260</td>
						<td>265</td>
						<td>270</td>
					</tr>
				
					<tr>
						<th>Japan (cm)</th>
						<td>22</td>
						<td>22.5</td>
						<td>23</td>
						<td>23.5</td>
						<td>24</td>
						<td>24.5</td>
						<td>25</td>
						<td>25.5</td>
						<td>26</td>
						<td>26.5</td>
						<td>27</td>
					</tr>

					<tr>
						<th>USA</th>
						<td>5</td>
						<td>5.5</td>
						<td>6</td>
						<td>6.5</td>
						<td>7</td>
						<td>7.5</td>
						<td>8</td>
						<td>8.5</td>
						<td>9</td>
						<td>9.5</td>
						<td>10</td>
					</tr>

					<tr>
						<th>Europe</th>
						<td>36</td>
						<td>36</td>
						<td>36.5</td>
						<td>37</td>
						<td>38</td>
						<td>38.5</td>
						<td>39</td>
						<td>40</td>
						<td>40.5</td>
						<td>41</td>
						<td>42</td>
					</tr>

					<tr>
						<th>UK</th>
						<td>3</td>
						<td>3.5</td>
						<td>4</td>
						<td>4.5</td>
						<td>5</td>
						<td>5.5</td>
						<td>6</td>
						<td>6.5</td>
						<td>7</td>
						<td>7.5</td>
						<td>8</td>
					</tr>
				</table>
			</div>
		</div>
		<button class="accordion">Men Shoes Size Chart</button>
		<div class="panel">
			<div class="table-wrapper">
				<h3>Men Shoes</h3>
				<table class="alt">
					<tr>
						<th>Korea (mm)</th>
						<td>240</td>
						<td>245</td>
						<td>250</td>
						<td>255</td>
						<td>260</td>
						<td>265</td>
						<td>270</td>
						<td>275</td>
						<td>280</td>
						<td>285</td>
						<td>290</td>
					</tr>
				
					<tr>
						<th>Japan (cm)</th>
						<td>24</td>
						<td>24.5</td>
						<td>25</td>
						<td>25.5</td>
						<td>26</td>
						<td>26.5</td>
						<td>27</td>
						<td>27.5</td>
						<td>28</td>
						<td>28.5</td>
						<td>29</td>
					</tr>

					<tr>
						<th>USA</th>
						<td>6</td>
						<td>6.5</td>
						<td>7</td>
						<td>7.5</td>
						<td>8</td>
						<td>8.5</td>
						<td>9</td>
						<td>9.5</td>
						<td>10</td>
						<td>10.5</td>
						<td>11</td>
					</tr>

					<tr>
						<th>Europe</th>
						<td>38</td>
						<td>38.5</td>
						<td>39</td>
						<td>40</td>
						<td>40.5</td>
						<td>41</td>
						<td>42</td>
						<td>42.5</td>
						<td>43</td>
						<td>44</td>
						<td>44.5</td>
					</tr>

					<tr>
						<th>UK</th>
						<td>5</td>
						<td>5.5</td>
						<td>6</td>
						<td>6.5</td>
						<td>7</td>
						<td>7.5</td>
						<td>8</td>
						<td>8.5</td>
						<td>9</td>
						<td>9.5</td>
						<td>10</td>
					</tr>
				</table>
			</div>
		</div>
		<button class="accordion">@lang('landingpage.t_reseller')</button>
		<div class="panel">
			<p>@lang('landingpage.t_reseller_1') <span><a href="#contact">@lang('landingpage.nav_contact')</a></span></p>
		</div>
</article>